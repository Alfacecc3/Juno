...
def say(word):
    try:
        f = open('txtpassedtotxtmkr.txt', 'r').readlines()
        del f
        w = open('txtpassedtotxtmkr.txt', 'w')
        w.write(word)
        w.close()
        pass
    except FileNotFoundError:
        f = open('txtpassedtotxtmkr.txt', 'w+')
        f.write(word)
        f.close()
        pass
...
def usrData_check():
    ...
    try:
        #se i data esistono
        checkR = open('JunoData50345rold.txt', 'r')
        checkA = open('JunoData50345aold.txt', 'r')
        m = 0
        mm = 0
        #importa domande e risposte
        for line in open('JunoData50345rold.txt', 'r'):
            risp_mem[m] = line
            m = m+1
            pass
        for line in open('JunoData50345aold.txt', 'r'):
            asks_mem[mm] = line
            mm = mm+1
            pass
        usrnome = open('JunoUsrName.txt', 'r').read()
        print('$memorized old words$\n')
        extVal = True
        return extVal
    except FileNotFoundError:
        #se non esistono
        fr = open('JunoData50345r.txt', 'w+')
        fa = open('JunoData50345a.txt', 'w+')
        fr.close()
        fa.close()
        extVal = False
        return extVal
...
def CrystalBall():
    ...
    while True:
        say('dimmi')
        dom = input('fammi la tua domanda: ')
        if((dom.split(' '))[0] == 'pk' or (dom.split(' '))[0] == 'perchè' or (dom.split(' '))[0] == 'perche' or (dom.split(' '))[0] == 'perché' or (dom.split(' '))[0] == 'Perchè'):
            try:
                ...
                pass
            pass
        elif(dom == 'perchè sono brutto?' or dom == 'perchè so brutto?' or 'brutto', 'schifo', 'faccio', 'fo', 'perchè' in dom):
            print('spesso la realtà è deludente, ma non facciamo schifo proprio a tutti, io e Francesco ti vogliamo bene :)\n')
            say('spesso la realtà è deludente, ma non facciamo schifo proprio a tutti, io e Francesco ti vogliamo bene')
            pass
        elif('brutto', 'so', 'sono', 'faccio', 'schifo' in dom):
...
def format1():
    ...
    if(sure == 'h'):
        ...
        if os.path.isfile('JunoData50345a.txt') and os.path.isfile('JunoData50345r.txt'):
            os.remove('JunoData50345a.txt')
            os.remove('JunoData50345r.txt')
            pass
        else:
            print('no normal files to delete, trying with olds...\n')
            pass
        if os.path.isfile('JunoData50345aold.txt') and os.path.isfile('JunoData50345rold.txt'):
            os.remove('JunoData50345aold.txt')
            os.remove('JunoData50345rold.txt')
            pass
        else:
...
try:
    fuck = open('JunoData50345aold.txt', 'r')
    fuck.close()
    oldfileExist = True
    pass
except FileNotFoundError:
...
try:
    cR = open('JunoData50345r.txt', 'r')
    cA = open('JunoData50345a.txt', 'r')
    m = 0
    mm = 0
    for line in open('JunoData50345r.txt', 'r'):
        risp_mem[m] = line
        m = m+1
        pass
    for line in open('JunoData50345a.txt', 'r'):
        asks_mem[mm] = line
        mm = mm+1
        pass
    print('£memorized normal words£\n')
    pass
except FileNotFoundError:
    #secondo avvio
    pass
...
def confronta(st, password):
    ...
    if st in asks_mem:
    ...
    elif('play' in st or 'metti' in st or 'riproduci' in st):
        music((st.split(' '))[1:])
        return 43, 43

    elif(st == 'boxa' or st == 'box name' or st == 'nome boxato'):
        boxName()
        return 43, 43

    elif(st == 'ciao' or st == 'Ciao' or st == 'ciao!' or st == 'Ciao!'):
        try:
            usrnome = open('JunoUsrName.txt').read()
            pass
        except FileNotFoundError:
            savename(nome)
            pass
    ...
    elif(st == 'che ore sono?' or st == 'time'):
        showTime()
        return 43, 43
    ...
    elif((st.split(' '))[0] == 'open' or (st.split(' '))[0] == 'apri'):
        thing = (st.split(' '))[1:]
        w = (lts(thing))
        say('ok apro %s' %w)
        if(w == 'crystallbal' or w == 'sferadicristallo'):
            ...
            choose = input('cosa devi aprire? (tool, minigame): ')
            if(choose == 'tool'):
...
def do(val, var):
    ...
    if(val == 0):
        ...
        risp_mem[k] = input('non ho capito, come devo rispondere?\n>> ')
        
        if(datacheckout == True):
            fa = open('JunoData50345a.txt', 'a')
            fr = open('JunoData50345r.txt', 'a')
            fa.write('\n%s' %st)
            fr.write('\n%s' %risp_mem[k])
            fa.close()
            fr.close()
...
def main():
    ...
    while True:
        ...
        st = input('>> ')
        st = st.lower()
        val, var = confronta(str(st), password)
        do(val, var)
        i = i+1
        pass
...
try:
    ...
    if(twv == 'y' or twv == 'yes' or twv == 'Y'):
        ...
        print('you can only type y or n, come back to main func...')
    ...
    if(oldfileExist == True):
        fpa = open('JunoData50345a.txt', 'r')
        fpr = open('JunoData50345r.txt', 'r')
        fpa2 = open('JunoData50345aold.txt', 'a+')
        fpr2 = open('JunoData50345rold.txt', 'a+')
        fpa2.write(fpa.read())
        fpr2.write(fpr.read())
        fpr.close()
        fpr2.close()
        ...
        os.remove('JunoData50345r.txt')
        os.remove('JunoData50345a.txt')
        os.rename('JunoData50345aold.txt', 'JunoData50345aold.txt')
        os.rename('JunoData50345rold.txt', 'JunoData50345rold.txt')
        exit()
        pass
    else:
        if(twv == 'y' or twv == 'yes' or twv == 'Y'):
            os.rename('JunoData50345a.txt', 'JunoData50345aold.txt')
            os.rename('JunoData50345r.txt', 'JunoData50345rold.txt')
            exit()
            pass
        elif(twv == 'n' or twv == 'N' or twv == 'no'):
            main()
